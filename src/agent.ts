...
async function triageItem(item: InboxItem): Promise<ItemOutput> {
  return withItemContext(item.id, async () => {
    ...
    if (urgency === "P1") {
      ...
      let holdNote = "No slots found for immediate hold.";
      ...
      if (slots.data.length > 0) {
        ...
        holdNote = `Slot held: ${best.start} with ${best.provider_name} (hold ${held.data.hold_id}, expires ${held.data.expires_at}). Staff must confirm before patient is notified.`;
        taskIds.push(held.data.hold_id);
      }
      ...
      const draft = await draft_message({
        recipient,
        channel,
        body: `Hi, we received your message about today's appointment for ${intake.child_name ?? "your child"}. Our team is working on rescheduling options and will follow up with you shortly.`,
        language: "en",
      });
      draftReply = draft.args.body as string;
      ...
      return buildOutput(
        ...
        null,
        draftReply,
        "Front desk to confirm held slot and contact family to reschedule same-day appointment.",
        `Same-day cancellation received. Classified ${classification} / P1 per scheduling policy. Searched for patient record, found replacement slots, placed a reviewable hold for staff to confirm.`,
      );
    }
